 ...
 * `docs/proposals/local-first-gateway.md` §4.2): the local-gateway flag is
 * off, the plan resolved pure-BYOK (the cloud keeps its own fallback +
 * cooldown machinery, which must stay byte-identical to today), or the
 * plan fetch failed. No `x-openllm-pin-model` — the cloud runs its full
 * resolve.
 *
