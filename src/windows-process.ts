...
export const setAdmissionForTest = (fn: (() => void) | undefined) => {
  admissionForTest = fn;
};
...
export const ensureWindowsProcessAdmission = (): void => {
  ...
  try {
    ...
    if (process.arch !== "x64")
      throw new Error("unsupported Windows admission ABI");
    if (!admission) {
      const library = bindKernel(),
        k = library.symbols;
      const check = (ok: unknown, operation: string) => {
        if (!ok)
          throw new Error(
            `Windows process admission ${operation} failed (${k.GetLastError()})`,
          );
      };
      ...
      try {
        ...
        const limits = new BigUint64Array(18),
          bytes = new DataView(limits.buffer);
        ...
        check(
          k.AssignProcessToJobObject(handle, k.GetCurrentProcess()),
          "assign caller",
        );
    ...
    const { handle, library } = admission,
      k = library.symbols;
    const member = new Int32Array(1),
      limits = new BigUint64Array(18);
    if (
      !k.IsProcessInJob(k.GetCurrentProcess(), handle, library.ptr(member)) ||
      member[0] !== 1 ||
    ...
    const bytes = new DataView(limits.buffer),
      flags = bytes.getUint32(16, true);
    if (
      !(flags & 0x8) ||
      flags & 0x1800 ||
      bytes.getUint32(40, true) !== WINDOWS_PROCESS_LIMIT
    )
      throw new Error("Windows process admission policy mismatch");
  } catch (error) {
    rejected = new Error(
      `Windows process admission unavailable: ${error instanceof Error ? error.message : String(error)}`,
    );
    throw rejected;
  }
};
...
export const validWindowsAdmission = (value: unknown): boolean => {
  ...
  return (
    v.limit === WINDOWS_PROCESS_LIMIT &&
    v.scope === "process-tree" &&
    v.prebirth === true &&
    v.breakaway === false
  );
};
