...
import { exponentialBackoffMs } from "./state-file";
import { enableUsagePersistence } from "./usage-cache";
import { DAEMON_VERSION } from "./version";
import { registerScmStopWake } from "./windows-scm";

export type TRunDaemonMainOptions = {
  readonly scmControlled?: boolean;
  readonly requestStop?: () => void;
};

// Legacy stdio stop pipe (retired sidecar); in-process SCM uses scm-win instead.
const serviceControl =
  process.platform === "win32" &&
  process.env.OPENLLM_SERVICE_CONTROL === "stdio-v1";
...
// Until it's healthy, refresh retries back off exponentially with jitter
// (NET-2): a boot-time `unreachable` or a `next dev` still compiling recovers
// in seconds, but a persistently failing cloud stops getting a fixed 5 s
// hammer from every unhealthy daemon in the fleet.
const BOOTSTRAP_RETRY_BASE_MS = 5 * 1000;
const BOOTSTRAP_RETRY_MAX_MS = 30 * 60 * 1000;
// A revoked/rejected key cannot heal by retrying — hold a long interval (with
// jitter, growing to the cap) instead of re-pounding the auth endpoint.
const BOOTSTRAP_INVALID_KEY_BASE_MS = 60 * 60 * 1000;
const BOOTSTRAP_INVALID_KEY_MAX_MS = 6 * 60 * 60 * 1000;
// `no_key` throws before any network call (authHeaders short-circuits), so its
// tick is local-only. It still backs off (a daemon may sit unprovisioned for
// days), but with a LOW cap: a just-provisioned key must be picked up within
// a minute, not after the network-failure schedule.
const BOOTSTRAP_NO_KEY_MAX_MS = 60 * 1000;

/**
 * Delay before the next bootstrap refresh for the CURRENT snapshot state and
 * consecutive-failure count. Exported for tests. `ok` relaxes to the TTL;
 * `invalid_key` waits out a long interval; `unreachable` uses jittered
 * exponential backoff capped at 30 min; `no_key` (a local-only probe) backs
 * off to a low cap so key pickup stays quick.
 */
export const nextBootstrapDelay = (
  state: TCloudState,
  failures: number,
): number => {
  if (state === "ok") return BOOTSTRAP_TTL_MS;
  if (state === "invalid_key") {
    return exponentialBackoffMs(
      failures,
      BOOTSTRAP_INVALID_KEY_BASE_MS,
      BOOTSTRAP_INVALID_KEY_MAX_MS,
    );
  }
  if (state === "no_key") {
    return exponentialBackoffMs(
...
export const runDaemonMain = async (
  options: TRunDaemonMainOptions = {},
): Promise<void> => {
  const scmControlled = options.scmControlled === true;

  // Crash-loop circuit breaker — the FIRST statement in this function. The
  // supervisor (systemd `Restart=always` / launchd `KeepAlive`) relaunches us
  // on every exit, so ANY crash this function hits before the guard —
  // `daemonPort()`'s env-file parse, the doctor cursor init — would restart
  // forever with no tally ever growing (and `--version` never exercises this
  // path, so the pre-swap probe can't catch it). Record this boot and, if
  // we've restarted too many times in a short window, disable self-restore
  // and exit cleanly so the thrashing STOPS (recover with `openllmd restart`).
  // systemd ALSO has a native start-limit backstop (`service.ts`); this is the
  // cross-platform half (launchd has no equivalent). Runs before the sandbox
  // so it can't be tripped by an FS-confinement bug, and before the (costly)
  // sandbox/FFI work it's meant to stop repeating.
  guardCrashLoop();
  ...
  const exitAfterDisposableDrain = (code: number): void => {
    void drainDisposableChildren().finally(() => {
      if (scmControlled) {
        notifyScmShutdownComplete(code);
        return;
      }
      process.exit(code);
    });
  ...
  // Consecutive refreshes that left the snapshot non-`ok` — feeds the backoff
  // schedule; resets to 0 the moment a refresh succeeds.
  let bootstrapFailures = 0;
  const noteBootstrapOutcome = (): void => {
    bootstrapFailures = getCloudState() === "ok" ? 0 : bootstrapFailures + 1;
  };
  const scheduleBootstrap = (): void => {
    const delay = nextBootstrapDelay(getCloudState(), bootstrapFailures);
    setTimeout(async () => {
      try {
  ...
  if (scmControlled && options.requestStop !== undefined) {
    registerScmStopWake(() => shutdown("SIGTERM"));
  } else if (serviceControl) {
    bindServiceControl(process.stdin, () => shutdown("SIGTERM"));
  }
  ...
  try {
    Bun.serve({
      ...
      fetch: async (
        ...
        // Authenticated local clients only: a browser WebSocket cannot set
        // Authorization, so browsers stay on the mux realtime1 channel.
        if (url.pathname === "/v1/realtime") {
          ...
          const upgraded = server.upgrade(req, {
            data: { open, handle: null, closed: false, pending: [] },
          });
          return upgraded
            ? undefined
      ...
      // The only local WS surface: `/v1/realtime`. Every upgrade already
      // carried a validated `open` payload in `data`.
  ...
  if (scmControlled) notifyScmListening();
};
