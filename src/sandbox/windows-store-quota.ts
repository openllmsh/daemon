 ...
 * platform-independent decision logic behind `native/quota-volume-helper.cs`.
 *
 * Two things live here, deliberately split from the native side so they can
 * be exercised by `bun test` on any host (the native side needs a real
 * Windows kernel and admin privilege — see `native/quota-volume-helper.cs`'s
 * header — and is exercised by a hardware probe instead, following the
 * `repair/*-test.ts` pattern used elsewhere in this codebase):
 *
 ...
 *     shape of `RecoverStaleLocked`/`RecoveryAcl`/`LeaseText` in
 *     `native/windows-appcontainer.cs` so the two recovery passes (task root,
...
export class QuotaExceededError extends Error {
  constructor(
    ...
    super(`store "${storeId}" refused ${requestedBytes} bytes: budget is ${budgetBytes} bytes`);
  }
}

/** What a kernel-backed store must provide. `native/quota-volume-helper.cs`
 *  implements this contract against a real fixed-size NTFS volume; tests use
...
export class FakeKernelVolume implements VolumeBackend {
  ...
  constructor(
    ...
    if (!Number.isInteger(budgetBytes) || budgetBytes <= 0) throw new Error("budget must be a positive integer");
  }
  get usedBytes(): number {
    return this.#used;
  }
  write(bytes: number): void {
    if (!Number.isInteger(bytes) || bytes < 0) throw new Error("write size must be a nonnegative integer");
    if (this.#used + bytes > this.budgetBytes) throw new QuotaExceededError(this.storeId, bytes, this.budgetBytes);
    this.#used += bytes;
  }
}
...
export interface QuotaVolumeRecord {
   ...
   *  before mounting completes. Mirrors `LeaseText` in
   *  native/windows-appcontainer.cs. */
  leaseText?: string;
}
...
export type LeaseVerdict = "ok" | "locked-invalid" | "malformed" | "alias-or-reparse" | "legacy";
...
export const classifyLeaseRecord = (input: {
  ...
  if (version !== "openllm-quota-volume-v1") return version.startsWith("openllm-task-recovery") ? "legacy" : "malformed";
  if (owner !== input.expectedOwner || storeId !== input.expectedStoreId) return "malformed";
  return "ok";
};
...
export class RecoveryLedger {
  ...
  static fromJSON(json: string): RecoveryLedger {
    ...
    for (const record of parsed) ledger.records.set(record.storeId, { ...record });
    return ledger;
  }
  toJSON(): string {
  ...
  beginCreate(storeId: string, owner: string, budgetBytes: number): void {
    if (this.records.has(storeId)) throw new Error(`store "${storeId}" already tracked`);
    this.records.set(storeId, { storeId, owner, budgetBytes, state: "creating" });
  }
   ...
   * code path).
   */
  recoverStale(liveOwners: ReadonlySet<string>, deadlineMs: number, now: () => number = Date.now): string[] {
    const start = now();
    const recovered: string[] = [];
    for (const record of this.records.values()) {
      if (record.state === "released" || liveOwners.has(record.owner)) continue;
      if (now() - start > deadlineMs) throw new Error("quota recovery deadline exceeded");
  ...
  isFullyReleased(): boolean {
    return [...this.records.values()].every(record => record.state === "released");
  }
}
