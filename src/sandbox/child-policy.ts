...
import { daemonWorkingSet, daemonTempDir, type TWorkingSet } from "./working-set";
...
export const childProtectedPaths = (home?: string): string[] => {
  ...
  const paths = [state, ...(explicit && isAbsolute(explicit) ? [explicit] : [])];
...
export const childWorkingSet = (home?: string): TWorkingSet => {
  const ws = daemonWorkingSet(home), protectedPaths = childProtectedPaths(home);
  const state = canonical(stateDir(home));
  const overlaps = (p: string): boolean => [resolve(p), canonical(p)].some(a =>
    protectedPaths.some(b => beneath(a, b) || beneath(b, a)));
  const broadRuntime = new Set(["/proc", "/run", "/var"]);
  const readOnly = ws.readOnly.filter(p => !broadRuntime.has(p) && !overlaps(p));
  const runtime = process.platform === "darwin" ? ["/System", "/Library/Apple", "/Library/Developer", "/Library/Frameworks", "/private/etc", "/private/var/select", "/var/select"] : ["/run/systemd/resolve", "/run/resolvconf"];
  ...
  if (process.platform === "darwin") {
    ...
    if (isAbsolute(developer) && existsSync(developer) && !overlaps(developer) && canonical(developer) !== "/" && statSync(developer).uid === 0) {
      ...
      if (developer.endsWith(".app/Contents/Developer")) {
        for (const name of ["Info.plist", "version.plist", "SharedFrameworks", "Frameworks"]) {
          const support = join(dirname(developer), name);
          if (existsSync(support) && !overlaps(support)) readOnly.push(support, canonical(support));
        }
      }
    }
    if (existsSync("/private/var/db/xcode_select_link")) readOnly.push("/private/var/db/xcode_select_link", "/var/db/xcode_select_link");
  }
  const readWrite = ws.readWrite.filter(p => canonical(p) !== canonical(dirname(process.execPath)) && !overlaps(p));
  ...
  if (canonical(scratch) !== join(state, "tmp")) throw new Error("Aliased child scratch rejected");
  readWrite.push(scratch);
  const selectedHome = process.env.HOME;
  for (const provider of CLI_PROVIDERS) {
    const root = join(state, "cli", provider), expected = join(root, "home");
    if (selectedHome !== expected && selectedHome !== join(stateDir(home), "cli", provider, "home")) continue;
    if (!existsSync(expected) || canonical(root) !== root || canonical(expected) !== expected) throw new Error("Aliased provider home rejected");
    readWrite.push(root);
  }
  return { readOnly: [...new Set(readOnly)], readWrite: [...new Set(readWrite)] };
};
...
export const childEnvironment = (env: NodeJS.ProcessEnv): NodeJS.ProcessEnv => {
  ...
  for (const name of Object.keys(child)) {
    if (/^(?:OPENLLM_|PRIVATE_PLANE_|RELAY_|DEVICE_GRANT_)/i.test(name)) delete child[name];
  }
  return child;
};
