...
export const isSandboxRejectionResponse = (response: Response): boolean =>
  terminalRejections.has(response);
...
export const sandboxSpawnArgs = (
  ...
  if (opts?.profile !== undefined) {
    if (
      ...
      throw new SandboxLaunchError(
        "POLICY_INVALID",
        "invalid uncredentialed task operation",
      );
    if (
      opts.network !== undefined ||
      (opts.extraReadWrite?.length ?? 0) > 0 ||
      (opts.extraReadOnly?.length ?? 0) > 0
    )
      throw new SandboxLaunchError(
        "PROFILE_UNSUPPORTED",
        "task profile grants cannot be overridden",
      );
    if (
      process.platform !== "win32" ||
      process.env.OPENLLM_DAEMON_NO_SANDBOX === "1"
    )
      throw new SandboxLaunchError(
        "SANDBOX_UNAVAILABLE",
        "Windows task confinement is unavailable or disabled",
      );
    return [
      process.execPath,
      ...(isDevSourceRun() && DEV_ENTRY ? [DEV_ENTRY] : []),
      "--sandbox-exec",
      "--profile",
      opts.profile,
      "--",
  ...
  if (opts?.required === true && opts.probe === true) {
    throw new SandboxLaunchError(
      "POLICY_INVALID",
      "required confinement conflicts with a probe exemption",
    );
  }
  if (
    opts?.network !== undefined ||
    ...
    throw new SandboxLaunchError(
      "PROFILE_UNSUPPORTED",
      "per-child path and network policy is not implemented",
    );
  }
  ...
  if (
    ...
    throw new SandboxLaunchError(
      "SANDBOX_UNAVAILABLE",
      "no qualified confinement backend on this platform",
    );
  }
  ...
  if (required && process.env.OPENLLM_DAEMON_NO_SANDBOX === "1") {
    throw new SandboxLaunchError(
      "SANDBOX_UNAVAILABLE",
      "required confinement is disabled",
    );
  }
  ...
  if (isDevSourceRun()) {
    ...
    if (process.env.OPENLLM_DAEMON_SANDBOX !== "1" && !required)
      return [...argv];
    if (DEV_ENTRY === undefined) {
      if (required)
        throw new SandboxLaunchError(
          "SANDBOX_UNAVAILABLE",
          "missing sandbox entry point",
        );
      return [...argv];
    }
    return [
...
export const runSandboxExec = async (
  ...
  if (opts?.profile !== undefined) {
    ...
    sandboxSpawnArgs(tail, {
      required: true,
      profile: opts.profile as "windows-cmd-v1",
    });
