import { childWorkingSet, childProtectedPaths } from "./child-policy";
/**
 ...
 */
const buildProfile = (home: string, homeOverride?: string, child = false): string => {
  const ws = child ? childWorkingSet(homeOverride) : daemonWorkingSet(homeOverride);
  const excluded = child ? childProtectedPaths(homeOverride) : [];
  const excluding = (rule: string): string => excluded.length ? `(require-all ${rule} ${excluded.map(p => `(require-not (subpath "${esc(p)}"))`).join(" ")})` : rule;
  const writeAllow = [...ws.readWrite.map(p => `  (subpath "${esc(p)}")`), ...(child ? [] : macRuntimeWrite(home)).map(p => `  ${excluding(`(subpath "${esc(p)}")`)}`)]
    .join("\n");
  ...
  const readAllow = [...ws.readWrite, ...ws.readOnly, ...(child ? [] : macHomeRead(home))]
  ...
  // contents.
  const metadataPaths = child ? [...new Set([...ws.readWrite, ...ws.readOnly].flatMap(p => { const paths: string[] = ["/var", "/tmp", "/etc"]; let cur = dirname(p); for (;;) { paths.push(cur); const parent = dirname(cur); if (parent === cur) break; cur = parent; } return paths; }))] : homeAncestorPaths([...ws.readWrite, ...ws.readOnly, ...macRuntimeWrite(home)].filter(inHome), home);
  const metadataAllow = metadataPaths
    .map((p) => `  (literal "${esc(p)}")`)
    .join("\n");
  return `(version 1)
(allow default)
${child ? `(deny process-info*)
 ...
 */
export const applySeatbelt = (homeOverride?: string, child = false): TSandboxState => {
