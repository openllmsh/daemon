...
import { startWindowsService, stopWindowsService, uninstallWindowsService, windowsSupervisor } from "./windows-service";
...
export const serviceStop = (): void => {
  if (process.platform === "win32") { stopWindowsService(); return; }
  if (isMac) stopMac();
  else stopLinux();
...
export const serviceStatus = async (): Promise<void> => {
  const registered = process.platform === "win32" ? windowsSupervisor().registered : isMac ? existsSync(plistPath()) : existsSync(unitPath());
