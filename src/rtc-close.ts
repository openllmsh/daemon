...
export const closeRtcPeer = async (
  ...
  try {
    ...
    const pendingReset = () => Boolean(sctp?.reconfigRequest) || (sctp?.reconfigQueue.length ?? 0) > 0;
    if (dc !== undefined && dc !== null && (dc.readyState === "open" || dc.readyState === "closing" || pendingReset())) {
      await new Promise<void>((resolve) => {
        ...
        const grace = typeof rto === "number" && Number.isFinite(rto)
          ? Math.min(4_000, Math.max(1_000, rto * 1_000 + 500)) : 1_000;
    ...
    unsubscribe?.();
    try { await pc.close(); } catch { /* already closed */ }
  }
};
