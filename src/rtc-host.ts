...
import { enforceRtcSeedGate, onDeviceAccessAuthorityChange } from "./device-access-verify";
...
const rtcHandshakeTimeoutMs = (): number => {
  ...
  if (!/^\d+$/.test(raw) || !Number.isSafeInteger(parsed) || parsed < 1_000) {
    logWarn("rtc-host", "invalid OPENLLM_RTC_HANDSHAKE_TIMEOUT_MS; using default", {
