...
export const assertRtcDependency = (): void => {
  ...
  try {
    ...
    if (
      ...
      !(receiver.pending instanceof Map) ||
      !(receiver.abort instanceof AbortController)
    ) {
      throw new Error(
        "Missing pinned werift DTLS repair; run bun install --frozen-lockfile in daemon before building",
      );
    }
  } finally {
    socket.close();
